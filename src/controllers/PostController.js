...
export const createPost = async (req, res) => {
  try {
    ...
    if( !user || !description || !interest) return responses.responseGeneric(res, 400, 'You must enter all the data')
...
export const updatePost = async (req, res) => {
  try {
    ...
    const updatePost = await Post.findOneAndUpdate({ _id: id, status: true }, {
      $set: {
...
const deletePost = async (req, res) => {
  try {
    ...
    const { id } = req.params
    const deletePost = await Post.findOneAndUpdate({ _id: id, status: true }, {
      $set: {
        status: false,
        deletedAt: new Date()
      }
...
export const setReactionPost = async (req, res) => {
  try {
    ...
    if (!post) {
      ...
      const updateReaction = await ReactionPost.findOneAndUpdate({ post: id }, {
        $set: {
          users: users
        }
      })
      ...
      responses.responseOK(res, 200, 'Success', reactionPost)
...
export const getReactionPost = async (req, res) => {
  try {
    ...
    const { id } = req.params
    const reactions = await ReactionPost.find({ post: id, status: true })
      .populate('users')
